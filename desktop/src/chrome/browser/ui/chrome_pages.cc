...
//netboxcomment begin
void ShowNetboxWallet(Browser* browser, bool create_new_tab, const std::string& page_name) {

    GURL url;
    
    if (page_name.empty())
    {
        url = GURL("chrome://wallet");   
    }
    else
    {
        url = GURL("chrome://wallet?" + page_name);   
    }

    if (true == create_new_tab)
    {
        NavigateParams params(GetSingletonTabNavigateParams(browser, url));
        params.path_behavior = NavigateParams::IGNORE_AND_NAVIGATE;
...
void ShowBinanceButton(Browser* browser)
{
    NavigateParams params(GetSingletonTabNavigateParams(browser, GURL("https://www.binance.org/")));
    params.path_behavior = NavigateParams::IGNORE_AND_NAVIGATE;

    ShowSingletonTabOverwritingNTP(browser, &params);
}

void ShowNetboxStore(Browser* browser, bool create_new_tab, const std::string& page_name) {

    GURL url;
    
    if (page_name.empty())
    {
        url = GURL("chrome://dapp-store");   
    }
    else
    {
        url = GURL("chrome://dapp-store?" + page_name);   
    }
