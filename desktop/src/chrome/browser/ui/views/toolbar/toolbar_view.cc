...
ToolbarView::ToolbarView(Browser* browser, BrowserView* browser_view)
  ...
  if (display_mode_ == DisplayMode::NORMAL) {
    ...
    // netboxcomment begin
    chrome::AddCommandObserver(browser_, IDC_NETBOX_WALLET_BUTTON, this);
    chrome::AddCommandObserver(browser_, IDC_BINANCE_BUTTON, this);
    // netboxcomment end    
...
ToolbarView::~ToolbarView() {
  ...
  // netboxcomment begin
  chrome::RemoveCommandObserver(browser_, IDC_BINANCE_BUTTON, this);
  chrome::RemoveCommandObserver(browser_, IDC_NETBOX_WALLET_BUTTON, this);
  // netboxcomment end
}
...
void ToolbarView::Init() {
  ...
  // netboxglobal begin
  netbox_wallet_button_ = new NetboxWalletButton(base::BindRepeating(callback, browser_, IDC_NETBOX_WALLET_BUTTON), u"Loading");
  netbox_wallet_button_->SetTriggerableEventFlags(ui::EF_LEFT_MOUSE_BUTTON | ui::EF_MIDDLE_MOUSE_BUTTON);
  netbox_wallet_button_->set_tag(IDC_NETBOX_WALLET_BUTTON);
  netbox_wallet_button_->SetID(VIEW_ID_NETBOX_WALLET_BUTTON);
  netbox_wallet_button_->SetTooltipText(u"Netbox.Wallet");
  netbox_wallet_button_->SetAccessibleName(l10n_util::GetStringUTF16(IDS_ACCNAME_RELOAD));
  netbox_wallet_button_->SetText(u"Loading");

  const ui::ThemeProvider* theme_provider = GetThemeProvider();
  if (theme_provider) 
  {
        const SkColor color = theme_provider->GetColor(ThemeProperties::COLOR_BOOKMARK_TEXT);
        netbox_wallet_button_->SetEnabledTextColors(color);  
  }

  std::unique_ptr<ToolbarButton> binance_button = std::make_unique<ToolbarButton>(
      base::BindRepeating(callback, browser_, IDC_BINANCE_BUTTON));

  binance_button->SetTriggerableEventFlags(ui::EF_LEFT_MOUSE_BUTTON | ui::EF_MIDDLE_MOUSE_BUTTON);
  binance_button->set_tag(IDC_BINANCE_BUTTON);
  binance_button->SetTooltipText(u"Binance exchange platform");
  binance_button->SetID(VIEW_ID_NETBOX_BINANCE_BUTTON);
  //??binance_button->SetInkDropMode(views::InkDropHostView::InkDropMode::ON);
  binance_button->SetInstallFocusRingOnFocus(true);
  binance_button->SetHasInkDropActionOnClick(true);
  //??binance_button->SetInkDropVisibleOpacity(0.06f);
  binance_button->SetLayoutInsets(gfx::Insets(2, 2));
  // netboxglobal end
  ...
  bool show_avatar_toolbar_button = false; // netboxcomment
#if BUILDFLAG(IS_CHROMEOS_ASH)
  ...
  binance_button_ = AddChildView(std::move(binance_button));// netboxcomment
  ...
  AddChildView(netbox_wallet_button_);// netboxcomment
...
void ToolbarView::EnabledStateChangedForCommand(int id, bool enabled) {
  ...
  // netboxcomment begin
  if (id == IDC_NETBOX_WALLET_BUTTON && netbox_wallet_button_)
  {
  	netbox_wallet_button_->SetEnabled(enabled);
  	return;
  }
  if (id == IDC_BINANCE_BUTTON && binance_button_)
  {
  	binance_button_->SetEnabled(enabled);
  	return;
  }
  // netboxcomment end
...
void ToolbarView::LoadImages() {
  ...
  if (tp)
  {
      SkColor normal_color   = tp->GetColor(ThemeProperties::COLOR_TOOLBAR_BUTTON_ICON);
      SkColor disabled_color = tp->GetColor(ThemeProperties::COLOR_TOOLBAR_BUTTON_ICON_INACTIVE);
      

      const gfx::ImageSkia& wallet_normal_image = gfx::CreateVectorIcon(vector_icons::kWalletIcon, 26, normal_color);
      const gfx::ImageSkia& wallet_disabled_image = gfx::CreateVectorIcon(vector_icons::kWalletIcon, 26, disabled_color);

      netbox_wallet_button_->SetImage(views::Button::STATE_NORMAL, wallet_normal_image);
      netbox_wallet_button_->SetImage(views::Button::STATE_DISABLED, wallet_disabled_image);

      const gfx::ImageSkia& binance_normal_image = gfx::CreateVectorIcon(vector_icons::kBinanceIcon, 24, normal_color);
      const gfx::ImageSkia& binance_disabled_image = gfx::CreateVectorIcon(vector_icons::kBinanceIcon, 24, disabled_color);

      binance_button_->SetImage(views::Button::STATE_NORMAL, binance_normal_image);
      binance_button_->SetImage(views::Button::STATE_DISABLED, binance_disabled_image);
  }
  // netboxglobal end
