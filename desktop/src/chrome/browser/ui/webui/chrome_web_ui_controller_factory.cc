...
WebUIFactoryFunction GetWebUIFactoryFunction(WebUI* web_ui,
  ...
  //netboxcomment begin
  if (url.host_piece() == chrome::kChromeUIWalletHost)
    return &NewWebUI<WalletUI>;
  if (url.host_piece() == chrome::kChromeUIWalletDebugHost)
    return &NewWebUI<WalletDebugUI>;
  if (url.host_piece() == chrome::kChromeUINetboxinfoHost)
    return &NewWebUI<NetboxinfoUI>;
  if (url.host_piece() == chrome::kChromeUIDapstoreHost)
    return &NewWebUI<DapstoreUI>;
  //netboxcomment end
...
base::RefCountedMemory* ChromeWebUIControllerFactory::GetFaviconResourceBytes(
  ...
  //netboxcomment begin
  if (page_url.host_piece() == chrome::kChromeUIWalletHost)
    return WalletUI::GetFaviconResourceBytes(scale_factor);

  if (page_url.host_piece() == chrome::kChromeUINetboxinfoHost)
    return NetboxinfoUI::GetFaviconResourceBytes(scale_factor);

  if (page_url.host_piece() == chrome::kChromeUIWelcomeHost)
    return WelcomeUI::GetFaviconResourceBytes(scale_factor);

  if (page_url.host_piece() == chrome::kChromeUIDapstoreHost)
    return DapstoreUI::GetFaviconResourceBytes(scale_factor);
  //netboxcomment end
