...
#include "base/environment.h" // netbox
namespace {
...
content::WebUIDataSource* CreateNewTabPageUiHtmlSource(
  ...
  std::string netboxqa_env_value;
  env->GetVar("NETBOXQA", &netboxqa_env_value);
  source->AddBoolean("netboxqa", "1" == netboxqa_env_value);
  // netboxcomment end
  ...
  source->AddBoolean("modulesEnabled",
					false); /* change to false, netbox comment begin
  ...
  source->OverrideContentSecurityPolicy(
      ...
      base::StringPrintf("child-src https: %s %s %s %s;", // netboxcomment
                         "cryptonews.netbox.global",  // netboxcomment
                         "devcryptonews.netbox.global", // netboxcomment
