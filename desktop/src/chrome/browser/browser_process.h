...
// complete.
#if defined(OS_WIN) || defined(OS_MAC) || (defined(OS_LINUX) || BUILDFLAG(IS_CHROMEOS_LACROS)) // netboxcomment include OS_MAC
  ...
  //netboxcomment begin
  virtual Netboxglobal::WalletManager* wallet_manager() const = 0;
  virtual Netboxglobal::WalletSessionManager* env_controller() const = 0;
  virtual Netboxglobal::TransactionService* transaction_service() const = 0;
  //netboxcomment end
