...
([I18nBehavior], PolymerElement) {
  ...
  ready() {
    ...
    {
        if(!event.target.closest('#netbox__frame'))
        {
            let netbox_iframe = $$(this, '#netbox__frame');
            if (netbox_iframe)
            {
                netbox_iframe.contentWindow.postMessage({cmd: 'hidelang'}, "*");
            }
        }
    });

    let netbox_iframe = document.createElement('iframe');
    netbox_iframe.id = 'netbox__frame';
    
    if (loadTimeData.getBoolean('netboxqa'))
    {
        netbox_iframe.src = 'https://devcryptonews.netbox.global/iframe.html'; // netboxcomment
    }
    else
    {
        netbox_iframe.src = 'https://cryptonews.netbox.global/iframe.html'; // netboxcomment
    }
    $$(this, '#netbox__frame-container').appendChild(netbox_iframe);

    netbox_iframe.onload = () =>
    {
        if (this.theme_)
        {
            const message = {cmd: 'updateTheme', tileTitleColor: skColorToRgba(this.theme_.shortcutTextColor)};
            netbox_iframe.contentWindow.postMessage(message, '*');
        }
    }
	// netboxcomment finish    
  }
