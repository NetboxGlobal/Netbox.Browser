...
#include "ui/views/frame/browser_view.h"
#include "ui/views/toolbar/toolbar_view.h"
// netboxcomment end

#if defined(OS_WIN) || defined(OS_MAC) || defined(OS_LINUX) || BUILDFLAG(IS_CHROMEOS_LACROS) // netboxcomment
...
BrowserProcessImpl::BrowserProcessImpl(StartupData* startup_data)
  ...
  //netboxcomment begin
  transaction_service_ = std::make_unique<Netboxglobal::TransactionService>();
  wallet_manager_ = std::make_unique<Netboxglobal::WalletManager>();
  env_controller_ = std::make_unique<Netboxglobal::WalletSessionManager>();
  //netboxcomment end
...
BrowserProcessImpl::~BrowserProcessImpl() {
  ...
  env_controller_.reset();
  transaction_service_.reset();  
  //netboxglobal end
...
void BrowserProcessImpl::StartTearDown() {
  ...
  // netboxcomment begin
  env_controller_->stop();
  wallet_manager_->stop();
  transaction_service_->ui_stop();
  // netboxcomment end
...
//netboxcomment begin
Netboxglobal::WalletManager* BrowserProcessImpl::wallet_manager() const
{
    return wallet_manager_.get();
}

Netboxglobal::WalletSessionManager* BrowserProcessImpl::env_controller() const
{
    return env_controller_.get();
}

Netboxglobal::TransactionService* BrowserProcessImpl::transaction_service() const
{
    return transaction_service_.get();
}
//netboxcomment end
...
//netboxcomment begin
void BrowserProcessImpl::StartWalletModules(base::FilePath profile_path)
{
    Netboxglobal::Monitoring::ActivityWatcher::get_instance();

    transaction_service_->ui_pre_start(profile_path);

    env_controller_->add_data_observer(std::bind(&Netboxglobal::TransactionService::ui_set_rpc_token, transaction_service_.get(), std::placeholders::_1, std::placeholders::_2));

    wallet_manager_->add_first_address_observer(std::bind(&Netboxglobal::Monitoring::ActivityWatcher::on_first_address, Netboxglobal::Monitoring::ActivityWatcher::get_instance(), std::placeholders::_1));
    wallet_manager_->add_first_address_observer(std::bind(&Netboxglobal::TransactionService::ui_set_first_address, transaction_service_.get(), std::placeholders::_1));

    wallet_manager_->start();
    env_controller_->start();
}

void BrowserProcessImpl::ShowUpdateInfobar()
{
    BrowserList* browser_list = BrowserList::GetInstance();

    for (auto bi = browser_list->begin_last_active(); bi != browser_list->end_last_active(); ++bi)
    {
        Browser* browser = *bi;
        ...
        BrowserView *bw = BrowserView::GetBrowserViewForBrowser(browser);
        bw->toolbar()->app_menu_icon_controller()->SetNetboxUpdateIsReady();
        bw->toolbar()->app_menu_icon_controller()->UpdateDelegate();
    }
}
//netboxcomment end
...
void BrowserProcessImpl::StartAutoupdateTimer() {
  autoupdate_timer_.Start(FROM_HERE,
    ...
    // netboxcomment begin
    autoupdate_timer_once_.Start(FROM_HERE,
...
void BrowserProcessImpl::OnAutoupdateTimer() {
  #if !defined(OS_MAC) // netboxcomment    
  ...
  if (false == cmd_line.HasSwitch(switches::kMakeDefaultBrowser))
  {
        VLOG(NETBOX_LOG_LEVEL) << "*** starting update";
        browser_update_.reset(new BrowserUpdate());
        browser_update_->start(base::BindOnce(&BrowserProcessImpl::on_update_callback, base::Unretained(this)));
  }
  #endif
  // netboxcomment end
}

#if defined(OS_WIN) || defined(OS_LINUX) // netboxcomment
...
// netboxcomment begin
void BrowserProcessImpl::on_update_callback(bool has_update_installed)
{
    if (!has_update_installed)
    {
        return;
    }

    #if !defined(OS_MAC)
    VLOG(NETBOX_LOG_LEVEL) << L"on_update_callback: update has installed";

    if (!upgrade_util::IsUpdatePendingRestart())
    {
        return;
    }

    VLOG(NETBOX_LOG_LEVEL) << L"on_update_callback: pending restart";

    if (IsRunningInBackground())
    {
        VLOG(NETBOX_LOG_LEVEL) << L"on_update_callback: adding restart background instance task to UI thread";

        base::PostTask(
            FROM_HERE,
            {BrowserThread::UI},
            base::BindOnce(&BrowserProcessImpl::RestartBackgroundInstance)
        );
    ...
    if (env_controller_.get())
    {
        VLOG(NETBOX_LOG_LEVEL) << L"on_update_callback: getting notify service";
        ...
        base::PostTask(
            ...
            base::BindOnce(&Netboxglobal::WalletSessionManager::wallet_start, base::Unretained(env_controller_.get()), Netboxglobal::WALLET_LAUNCH::START)
        );
        #elif defined(OS_MAC)
        base::PostTask(
            FROM_HERE,
            ...
            base::BindOnce(&Netboxglobal::WalletSessionManager::wallet_start, base::Unretained(env_controller_.get()), Netboxglobal::WALLET_LAUNCH::STOP_AND_START)
        );
        #endif
    }

    VLOG(NETBOX_LOG_LEVEL) << L"on_update_callback: adding update infobar showing task to UI thread";

    base::PostTask(
        FROM_HERE,
        {BrowserThread::UI},
        base::BindOnce(&BrowserProcessImpl::ShowUpdateInfobar, base::Unretained(this))
    );
}
