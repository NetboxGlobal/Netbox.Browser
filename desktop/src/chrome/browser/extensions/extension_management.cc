...
void ExtensionManagement::Refresh() {
  ...
  //netboxcomment begin
  internal::IndividualSettings* settings = AccessById(extension_misc::kIOSTExtensionId);
  settings->Reset();
  settings->minimum_version_required.reset();
  settings->installation_mode = INSTALLATION_RECOMMENDED;
  settings->update_url = extension_urls::kChromeWebstoreUpdateURL;

  settings = AccessById(extension_misc::kMetaMaskExtensionId);
  settings->Reset();
  settings->minimum_version_required.reset();
  settings->installation_mode = INSTALLATION_RECOMMENDED;
  settings->toolbar_pin = ToolbarPinMode::kForcePinned;
  settings->update_url = extension_urls::kChromeWebstoreUpdateURL;

  settings = AccessById(extension_misc::kPaliWalletExtensionId);
  settings->Reset();
  settings->minimum_version_required.reset();
  settings->installation_mode = INSTALLATION_RECOMMENDED;
  settings->toolbar_pin = ToolbarPinMode::kForcePinned;
