...
void ExternalInstallManager::OnExtensionInstalled(
  ...
  //netboxcomment begin
  if (extension->id() == extension_misc::kMetaMaskExtensionId 
    || extension->id() == extension_misc::kPaliWalletExtensionId
    || extension->id() == extension_misc::kIOSTExtensionId)
  {
    AcknowledgeExternalExtension(extension->id());
    ExtensionSystem::Get(browser_context)->extension_service()->GrantPermissionsAndEnableExtension(extension);
    return;
  }
  //netboxcomment end
