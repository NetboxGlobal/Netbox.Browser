...
// netboxcomment begin
std::string netbox_replace(const std::string& s)
{
    std::regex rgx(
        // TODO, C++ doesn't support lookup behind
        //"(?<![a-zA-Z_@.\\/])(?<!Google )(?<!href=\"\\$[0-9]\">)(Chromium|Chrome)(?![a-zA-Z]|_|\\/|@)(?! OS)(?! Frame)(?! Web Store)(?![.][a-zA-Z])(?!\\<\\/a\\>)", 
        "(Chromium|Chrome)(?![a-zA-Z]|_|\\/|@)(?! OS)(?! Frame)(?! Web Store)(?![.][a-zA-Z])(?!\\<\\/a\\>)", 
        std::regex_constants::icase
    );

    return regex_replace(s, rgx, "NetboxBrowser");
}
// netboxcomment end
...
int main(int argc, char* const argv[]) {
  ...
  for (int loop = 0; loop < lang_list_count; ++loop) {
    ...
    std::map<std::string, std::string> infoplist_strings = {
        {"CFBundleGetInfoString", netbox_replace(get_info)},
        {"NSHumanReadableCopyright", netbox_replace(copyright)},
