...
def main(options):
  ...
  # netboxglobal begin
  path_correct_bin  = os.path.realpath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "..", "buildcorrectbin.php"))

  # remove autobuild path and sign
  filenames = ['chrome.exe', 'chrome.dll', 'chrome_elf.dll', 'libegl.dll', 'libglesv2.dll', 'setup.exe', 'nacl64.exe', 'chrome_proxy.exe', 'notification_helper.exe']
  for filename in filenames:
    if False == os.path.isfile(filename):
      raise Exception("can't find file %s", filename)
    RunSystemCommand("php.exe " + path_correct_bin + " -b=" + os.path.join(options.build_dir, filename), options.verbose);

  shutil.copy(os.path.join(options.build_dir, "chrome.exe"), os.path.join(options.build_dir, "netboxbrowser.exe"));
  # netboxglobal end
