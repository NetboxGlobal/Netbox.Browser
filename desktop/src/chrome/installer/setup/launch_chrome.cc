...
base::CommandLine GetPostInstallLaunchCommand(
    const base::FilePath& application_path, const base::CommandLine& options) { // netboxcomment
  base::CommandLine cmd(application_path.Append(kChromeExe));
  cmd.AppendSwitch(::switches::kFromInstaller);
  cmd.AppendArguments(options, false); // netboxcomment  
  return cmd;
}

bool LaunchChromeBrowser(const base::FilePath& application_path, const base::CommandLine& options) { // netboxcomment
  if (application_path.empty())
    return false;

  return base::LaunchProcess(GetPostInstallLaunchCommand(application_path, options),
                             base::LaunchOptions())
      .IsValid();
}
