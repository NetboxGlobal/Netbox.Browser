...
extern base::CommandLine get_netbox_launch_options(const installer::InstallerState &installer_state, const base::CommandLine& cmd_line);
// netboxcomment end
...
installer::InstallStatus UninstallProducts(InstallationState& original_state,
  ...
  {
    auto_launch_util::RealDisableBackgroundStartAtLogin();
  }
  // netboxcomment end
    ...
    if (proceed_with_installation) {
      ...
      if (install_status == SAME_VERSION_REPAIR_FAILED) {
        ...
        if (installer_state.target_path().empty()) {
          ...
          quoted_chrome_exe =
              GetPostInstallLaunchCommand(installer_state.target_path(), base::CommandLine(base::CommandLine::NO_PROGRAM)) // netboxcomment
      ...
      {
        wallet_install_or_update(installer_state.target_path(), installer_version);
      }
      // netboxcomment end
      ...
      if (install_status == FIRST_INSTALL_SUCCESS) {
        ...
        if (!system_install && !do_not_launch_chrome)
          LaunchChromeBrowser(installer_state.target_path(), get_netbox_launch_options(installer_state, cmd_line));//netboxcomment
