...
DeleteResult DeleteChromeFilesAndFolders(const InstallerState& installer_state,
  ...
  // close and delete browser
  CloseAllChromeProcesses(target_path);

  base::FilePath chrome_exe_path = target_path.Append(kChromeExe);
  base::DeleteFile(chrome_exe_path);
  if (base::PathExists(chrome_exe_path))
  {
    VLOG(1) << "pre, failed to delete (1st try) " << chrome_exe_path.value();
    ...
    if (base::PathExists(chrome_exe_path))
    {
        VLOG(1) << "pre, failed to delete (2st try) " << chrome_exe_path.value();
    }
  }

  // close wallet
  base::FilePath wallet_path; 
  if (Netboxglobal::get_wallet_exe_path(&wallet_path, FILE_PATH_LITERAL("")))
  {
        base::FilePath wallet_folder = installer_directory.Append(L"..").Append(L"wallet");
        VLOG(1) << L"deleting wallet " << wallet_path.value() << L" using wallet client from " << wallet_folder.value();

        send_stop_signal_to_wallet_and_wait("uninstall");
        base::DeleteFile(wallet_path);
  }

  // remove wallet autorun
  installer::remove_wallet_registry();  
...
InstallStatus IsChromeActiveOrUserCancelled(
  ...
  // netboxcoment begin
    const base::CommandLine* current_cmdline = base::CommandLine::ForCurrentProcess();
    if (current_cmdline && 
        (current_cmdline->HasSwitch(switches::kEnableLogging) || current_cmdline->HasSwitch(switches::kVerboseLogging)))
    {
        #if defined(OS_WIN)
...
InstallStatus UninstallProduct(const ModifyParams& modify_params,
  ...
  // constant should be taken from \chrome\installer\gcapi\gcapi.cc 
  HWND hwnd = FindWindow(L"NetboxGlobal_WidgetWin_1", NULL); // netboxtodo - we can replace it by const


  // close all processs in case we are in background mode
  if (NULL == hwnd || INVALID_HANDLE_VALUE == hwnd)
  {
		CloseAllChromeProcesses(target_path);
  }

  // check for the second window
  HWND hwnd2 = FindWindowEx(NULL, hwnd, L"NetboxGlobal_WidgetWin_1", NULL);
  if (hwnd2 == NULL)
  {
	  // get window style and check maximize button
	  LONG data = GetWindowLong(hwnd, GWL_STYLE);
	  if (0 == (data & WS_MAXIMIZEBOX))
	  {
		  CloseAllChromeProcesses(target_path);
	  }
  }
  ...
  //netboxcomment begin 
  //Remove guid value and checksum
  InstallUtil::DeleteRegistryKey(reg_root, std::wstring(L"Software\\") + std::wstring(install_static::kProductPathName), KEY_WOW64_32KEY);
  //netboxcomment end
  ...
  if (delete_profile) {
    ...
    installer::remove_wallet_files(); // netboxcomment
  }
