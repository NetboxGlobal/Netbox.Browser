...
base::CommandLine GetPostInstallLaunchCommand(
    const base::FilePath& application_path, const base::CommandLine& options); // netboxglobal

// Launches the browser following a successful install without waiting for it to
// exit.
bool LaunchChromeBrowser(const base::FilePath& application_path, const base::CommandLine& options); // netboxglobal
