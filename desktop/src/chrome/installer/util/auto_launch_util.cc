...
const wchar_t kAutolaunchKeyValue[] = L"NetboxBrowserAutoLaunch";
...
std::wstring GetAutoLaunchKeyName() {
  ...
  { 
     ...
     if (appdata)
     {
        path = base::FilePath(appdata);
        path = path.Append(install_static::kProductPathName).Append(chrome::kUserDataDirname);
     }
#endif
  } 
...
// netboxcomment begin
void RealDisableBackgroundStartAtLogin() {
  base::win::RemoveCommandFromAutoRun(HKEY_CURRENT_USER,
                                      GetAutoLaunchKeyName());
}
// netboxcomment end
