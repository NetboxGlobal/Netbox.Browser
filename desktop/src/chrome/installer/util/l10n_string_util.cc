...
class installer_tranlsation
{	
public:
	installer_tranlsation()
	{
		UErrorCode status = U_ZERO_ERROR;

		std::string pattern = "(?<![a-zA-Z_@.\\/])(?<!Google )(?<!href=\"\\$[0-9]\">)(Chromium|Chrome)(?![a-zA-Z]|_|\\/|@)(?! OS)(?! Frame)(?! Web Store)(?![.][a-zA-Z])(?!\\<\\/a\\>)"; 
		const icu::UnicodeString icu_pattern(pattern.c_str(), pattern.length());
		re_.reset(new icu::RegexMatcher(icu_pattern, 0, status));            

        if (status != U_ZERO_ERROR)
        {
            VLOG(1) << "pattern is not valid";
        }
	}

	static installer_tranlsation* GetInstance() {
		return base::Singleton<installer_tranlsation, base::LeakySingletonTraits<installer_tranlsation>>::get();
	}
	...
	bool translate(const std::wstring& src, std::wstring& dst)
	{
		if (!re_.get())
		{
			return false;
		...
		if(src.find_first_of(L"Chrome") == std::string::npos && src.find_first_of(L"Chromium") == std::string::npos)
		{
			return false;
		}

		auto it = translations_.find(src);
        ...
        icu::UnicodeString icu_input(src.c_str(), src.length());
		re_->reset(icu_input);
		
		UErrorCode status = U_ZERO_ERROR;
		icu::UnicodeString replaced_string = re_->replaceAll("Netbox", status);
		if (!U_SUCCESS(status))
		{    
			return false;
		}
		...
		std::string dst8;
		replaced_string.toUTF8String(dst8);
		dst = base::UTF8ToWide(dst8);

		translations_[src] = dst;
	   
		return true;    
	} 

 private:
	std::unique_ptr<icu::RegexMatcher> re_;
	std::map<std::wstring, std::wstring> translations_;  
};



std::wstring installer_translate(const std::wstring& src)
{
    installer_tranlsation* t = installer_tranlsation::GetInstance();
    if (!t)
    {
        return src;
    }
...
std::wstring GetLocalizedString(int base_message_id) {
  ...
  if (g_translation_delegate)
    return installer_translate(g_translation_delegate->GetLocalizedString(base_message_id)); // netboxcomment
  ...
  return installer_translate(localized_string); // netboxcomment
}
