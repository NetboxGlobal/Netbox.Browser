...
gen_changelog() {
  ...
  debchange -a --nomultimaint -m --changelog "${DEB_CHANGELOG}" \
    "" # netboxcomment
    # netbox "Release Notes: ${RELEASENOTES}"
...
do_package() {
  ...
  if [ ${IS_OFFICIAL_BUILD} -ne 0 ]; then
    ...
    local COMPRESSION_OPTS="-Zxz -z9" # netboxcomment same compression level as for official build
  fi
...
cd "${OUTPUTDIR}"
BASEREPOCONFIG="repo.netbox.global/apt/ stable main"
