...
// netboxcomment begin
ExtensionFunction::ResponseAction RuntimeNetboxOpenWalletFunction::Run()
{
    std::string page_raw;
    std::string page_verified;
    if (args_->GetString(0, &page_raw))
    {
        if ("transactions" == page_raw 
         || "send" == page_raw 
         || "receive" == page_raw 
         || "stake" == page_raw 
         || "buy" == page_raw 
         || "lottery" == page_raw 
         || "changewallet" == page_raw
         || "showwallet" == page_raw)
        {
            page_verified = page_raw;
        }
    }

    ChromeExtensionFunctionDetails details(this);
    Browser* browser = details.GetCurrentBrowser();
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE,
            base::BindOnce(&chrome::ShowNetboxWallet, browser, false, page_verified));

    return RespondNow(OneArgument(base::Value(true)));
}

ExtensionFunction::ResponseAction RuntimeNetboxOpenStoreFunction::Run()
{
    std::string page_raw;
    std::string page_verified;
    if (args_->GetString(0, &page_raw))
    {
        ...
        if (re2::RE2::FullMatch(page_raw, re2::RE2("^[a-zA-Z0-9]{64}$")))
        {
            page_verified = "txid=" + page_raw;
        }
    }
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE,
            base::BindOnce(&chrome::ShowNetboxStore, browser, false, page_verified));
