...
#include <Shlobj_core.h> //netboxcomment
...
//netboxcomment begin
#if defined(OS_WIN)
static PathString check_path(const PathString &path)
{

  LPWSTR desktop_path_raw;
  if (FAILED(SHGetKnownFolderPath(FOLDERID_Desktop, 0, NULL, &desktop_path_raw)))
      return path;

  PathString desktop_path = PathString(desktop_path_raw) + L"\\";
  ...
  const DWORD tmp_dir_len = MAX_PATH + 1;
  WCHAR tmp_dir_raw[tmp_dir_len];

  if (0 == GetTempPath(tmp_dir_len, tmp_dir_raw))
    return path;
  else
    return PathString(tmp_dir_raw) + L"\\";
}
#endif
//netboxcomment end
...
PathString GetDefaultLogFile() {
  ...
  log_name = check_path(log_name) + FILE_PATH_LITERAL("debug.log");//netboxcomment
  return log_name;
#elif defined(OS_POSIX) || defined(OS_FUCHSIA)
...
bool InitializeLogFileHandle() {
  ...
  if (g_log_file == INVALID_HANDLE_VALUE || g_log_file == nullptr) {
    ...
    *g_log_file_name = check_path(*g_log_file_name) + FILE_PATH_LITERAL("debug.log");//netboxcomment
