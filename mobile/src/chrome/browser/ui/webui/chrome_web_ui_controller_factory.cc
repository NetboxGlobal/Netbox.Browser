...
WebUIFactoryFunction GetWebUIFactoryFunction(WebUI* web_ui,
  ...
  //netboxcomment begin
  if (url.host_piece() == chrome::kChromeUIMobileWalletHost)
  {
    return &NewWebUI<MobileWalletUI>;
  }
  if (url.host_piece() == chrome::kChromeUIMobileWalletDebugHost)
  {
    return &NewWebUI<MobileWalletDebugUI>;
  }
  //netboxcomment end
...
void ChromeWebUIControllerFactory::GetFaviconForURL(
  ...
  // netboxcomment begin
  if (page_url.host_piece() == chrome::kChromeUIMobileWalletHost)
  {
    resource_scale_factors = {ui::SCALE_FACTOR_100P, ui::SCALE_FACTOR_200P, ui::SCALE_FACTOR_300P};
  }
  // netboxcomment end
...
base::RefCountedMemory* ChromeWebUIControllerFactory::GetFaviconResourceBytes(
  ...
  // netboxcomment begin
  if (page_url.host_piece() == chrome::kChromeUIMobileWalletHost
   || page_url.host_piece() == chrome::kChromeUIMobileWalletDebugHost)
    return MobileWalletUI::GetFaviconResourceBytes(scale_factor);
  // netboxcomment end
