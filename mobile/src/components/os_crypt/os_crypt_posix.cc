...
// Salt for Symmetric key derivation.
const char kNetboxSalt[] = "25415369-ea82-4e62-a95f-3fa8f5351d50";
...
const char kObfuscationPrefixNetbox[] = "v09";
...
crypto::SymmetricKey* GetEncryptionKeyNetbox(std::string password) {
  std::string salt(kNetboxSalt);
...
bool OSCrypt::EncryptStringNetbox(const std::string& plaintext,
  ...
  std::unique_ptr<crypto::SymmetricKey> encryption_key(GetEncryptionKeyNetbox(password));
  if (!encryption_key.get())
    return false;
  ...
  // Prefix the cypher text with version information.
  ciphertext->insert(0, kObfuscationPrefixNetbox);
  return true;
}
...
bool OSCrypt::DecryptStringNetbox(const std::string& ciphertext,
                            const std::string& password,
                            std::string* plaintext) {
  // This currently "obfuscates" by encrypting with hard-coded password.
  // We need to improve this password situation by moving a secure password
  ...
  // Credit card numbers are current legacy data, so false match with prefix
  // won't happen.
  if (!base::StartsWith(ciphertext, kObfuscationPrefixNetbox,
                        base::CompareCase::SENSITIVE)) {
    *plaintext = ciphertext;
    return true;
  }
  ...
  // Strip off the versioning prefix before decrypting.
  std::string raw_ciphertext = ciphertext.substr(strlen(kObfuscationPrefixNetbox));
