...
void WebUIImpl::CallJavascriptFunctionUnsafe(
  ...
  // netboxcomment begin
  void WebUIImpl::CallJavascriptEvent(base::Value event_name, base::Value event_params)
  {
    base::Value data(base::Value::Type::DICTIONARY);
    data.SetKey("detail", event_params.Clone());

    base::string16 result(base::ASCIIToUTF16("window.dispatchEvent(new CustomEvent("));

    std::string json;
    base::JSONWriter::Write(event_name, &json);
    result.append(base::UTF8ToUTF16(json));
    result.push_back(',');
    ...
    // LOG(WARNING) << "NETBOXREMOVE *** JS " << event_name.GetString() << " " << json.substr(0, 500);
