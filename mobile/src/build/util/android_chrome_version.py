...
def GenerateVersionCodes(version_values, arch, is_next_build):
  ...
  # netboxcomment begin
  netbox_version_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'chrome', 'NETBOX_VERSION')
  netbox_version = 0

  if os.path.exists(netbox_version_path):
    f = open(netbox_version_path, 'r')
    netbox_version = int(f.read())
    f.close()

  base_version_code = int('%s00' % netbox_version)
  # netboxcomment end
